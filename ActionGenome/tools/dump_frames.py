...
def dump_frames(args):
    ...
    for v in tqdm(video2frames):
        ...
        if not os.path.exists(curr_frame_dir) or len(video2frames[v]) != len(os.listdir(curr_frame_dir)):
            if os.path.exists(curr_frame_dir):
                for file_or_dir in os.listdir(curr_frame_dir):
                    file_or_dir_path = os.path.join(curr_frame_dir, file_or_dir)
                    if os.path.isdir(file_or_dir_path):
                        os.rmdir(file_or_dir_path)
                    else:
                        os.remove(file_or_dir_path)
                os.rmdir(curr_frame_dir)
