...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold = 0.9, **kwargs):
    ...
    """
    comparison = {}
    comparison['gt'] = gt_entry
    comparison['pred'] = pred_entry
    comparison['result_dict'] = result_dict
    with open("gt_pred/replace.txt", "w") as outfile:
        outfile.write(json.dumps(comparison))
