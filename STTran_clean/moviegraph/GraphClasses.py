...
if nx.__version__ != '1.11':
    print("Warning: This code was developed and tested with NetworkX 1.11, but you are using version {}.".format(nx.__version__))
    print("networkx-2 will NOT work because they changed many functions!")
else:
    print(f"You are using NetworkX {nx.__version__}")
...
class ClipGraph(object):
    ...
    def node_type(self, n, ntype=None):
        ...
        if ntype:
            #if n in self.G.nodes() and ntype == self.G.node[n]['type']: #Julie
            ...
            if n in self.G.nodes():
                #return self.G.node[n]['type'] #Julie
    ...
    def visualize_graph(self, identifier=None, prop_labels=False):
        ...
        'topic': '#FFDF0D',
        'time': '#9F9F9F',
        'new_attribute': '#EEFF00',
        'new_relationship': '#00FFAA',
        'scene': '#808080',
        'situation': '#808080'}
        ...
        my_nodes = list(self.G.nodes)
        list_nodes = list(my_nodes)
        n_ = list_nodes[1]
        print("Type : ",type(my_nodes))
        print("My nodes : ",my_nodes)
        print("Try degree : ", dict(self.G.degree()))
        print("Try accesing a node", self.G.nodes[n_])
        #    for n, v in list(self.G.nodes()): #Julie
        ...
        #new for loop : 
        for num_node in my_nodes:
            v = self.G.nodes[num_node]
            if 'propagated' in v.keys() and v['propagated']:
               colors.append(node_colors['new_' + v['type']])
            else:
               colors.append(node_colors[v['type']])
            node_positions[num_node] = v['pos']
            labels[num_node] = v['origtext']
            label_positions[num_node] = (v['pos'][0], v['pos'][1]+20)
        # do the plotting
        fig = plt.gcf()
